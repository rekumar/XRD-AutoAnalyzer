    ...
    def are_soluble(self, pair):
        ...
        if formula_A != formula_B:
            if self.matcher.fit(struc_A, struc_B):
                ...
                if (
                    len(probable_oxis_A) == 0
                ):  ## This occurs for metals, in which case we'll just use atomic radii
                    oxi_dict_A = {}
                ...
                for (site_A, site_B) in zip(struc_A, struc_B):
                    elem_A = "".join(
                        [char for char in str(site_A.species_string) if char.isalpha()]
                    )
                    elem_B = "".join(
                        [char for char in str(site_B.species_string) if char.isalpha()]
                    )
                    if solubility == True:
                        ...
                        if site_A_oxi.is_integer():
                            ...
                            possible_rA = []
                            possible_rA.append(
                                pt.Element(elem_A).ionic_radii[
                                    int(np.floor(site_A_oxi))
                                ]
                            )
                            possible_rA.append(
                                pt.Element(elem_A).ionic_radii[int(np.ceil(site_A_oxi))]
                            )
                        if site_B_oxi.is_integer():
                            ...
                            possible_rB = []
                            possible_rB.append(
                                pt.Element(elem_B).ionic_radii[
                                    int(np.floor(site_B_oxi))
                                ]
                            )
                            possible_rB.append(
                                pt.Element(elem_B).ionic_radii[int(np.ceil(site_B_oxi))]
                            )
                        possible_diffs = []
    ...
    def generate_solid_solns(self, pair, num_solns=3):
        ...
        if (struc_A is None) or (struc_B is None):
            ...
            interp_structs = struc_A.interpolate(
                struc_B, nimages=num_solns, interpolate_lattices=True
            )[1:]
